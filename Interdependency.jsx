...
export default function Interdependency({ t, kind = "feed" }) {
  ...
  const [view, setView] = useState("matrix"); // matrix | focus | swim | hub
  ...
  useEffect(() => {
    ...
    fn(domFilter).then((g) => {
      const graphData = g && g.nodes ? g : { nodes: [], edges: [], hubs: [] };
      setGraph(graphData);
      // collapse every lane by default so the swimlane opens clean; the user
      // expands the domains they want to investigate.
      const doms = [...new Set(graphData.nodes.map((n) => n.domain))];
      setCollapsedLanes(Object.fromEntries(doms.map((d) => [d, true])));
      setLoading(false);
    });
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <Chip t={t} active={view === "matrix"} onClick={() => { setView("matrix"); setSel(null); setHubFocus(null); }} label="Matrix" />
        <Chip t={t} active={view === "focus"} onClick={() => { setView("focus"); setSel(null); setHubFocus(null); }} label="Focus" />
        ...
        <div style={{ ...wrapStyle, flex: 1, minWidth: 0, overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px",
            ...
            <span style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".4px", color: t.sub }}>
              {view === "matrix" ? `${label} — domain × domain shared-key matrix`
                : view === "focus" ? `${label} — focus mode (pick one to see its links)`
          ...
          <div style={{ height: full ? "calc(100vh - 45px)" : 560, overflow: "auto" }}>
            ...
            {!loading && graph.nodes.length > 0 && view === "matrix" && (
              <MatrixView t={t} graph={graph} domains={domains} nodeById={nodeById}
                setSel={setSel} setView={setView} setDomFilter={setDomFilter} />
            )}
            {!loading && graph.nodes.length > 0 && view === "focus" && (
              <FocusView t={t} graph={graph} domains={domains} nodeById={nodeById}
...
// ---- Swimlane ----
// ---- Matrix view: domain × domain shared-key link counts ----
function MatrixView({ t, graph, domains, nodeById, setSel, setView, setDomFilter }) {
  // count cross-pair links per (domainA, domainB) and collect the pairs
  const counts = {}; const pairs = {};
  const key = (a, b) => [a, b].sort().join("|||");
  graph.edges.forEach((e, i) => {
    const na = nodeById(e.a), nb = nodeById(e.b); if (!na || !nb) return;
    const k = key(na.domain, nb.domain);
    counts[k] = (counts[k] || 0) + 1;
    (pairs[k] = pairs[k] || []).push(i);
  });
  const maxCount = Math.max(1, ...Object.values(counts));
  const cell = (rd, cd) => {
    if (rd === cd) return { n: null };
    const k = key(rd, cd);
    return { n: counts[k] || 0, edges: pairs[k] || [] };
  };
  const [hover, setHover] = useState(null);
  const shade = (n) => {
    if (!n) return t.bg;
    const ratio = n / maxCount;
    // interpolate light -> accent
    const r = Math.round(224 - ratio * (224 - 15));
    const g = Math.round(245 - ratio * (245 - 71));
    const b = Math.round(253 - ratio * (253 - 117));
    return `rgb(${r},${g},${b})`;
  };
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ fontSize: 12, color: t.sub, marginBottom: 12 }}>
        Each cell = how many shared-key links connect two domains. Darker = stronger. Click a cell to see those feed pairs in Focus.
      </div>
        ...
        <table style={{ borderCollapse: "collapse" }}>
          <thead>
            <tr>
              ...
              {domains.map((d) => (
                <th key={d} style={{ padding: "8px 6px", fontSize: 10, fontWeight: 700, color: domColor(domains, d),
                  writingMode: "vertical-rl", textAlign: "left", height: 90, whiteSpace: "nowrap" }}>{d}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {domains.map((rd) => (
              <tr key={rd}>
                <td style={{ padding: "6px 10px", fontSize: 11, fontWeight: 700, color: domColor(domains, rd),
                  textAlign: "right", whiteSpace: "nowrap" }}>{rd}</td>
                {domains.map((cd) => {
                  const c = cell(rd, cd);
                  const isDiag = c.n === null;
                  const hovered = hover === `${rd}:${cd}`;
                  return (
                    <td key={cd}
                      onMouseEnter={() => setHover(`${rd}:${cd}`)}
                      onMouseLeave={() => setHover(null)}
                      onClick={() => { if (c.n) { setDomFilter(rd); setView("focus"); } }}
                      style={{ width: 46, height: 40, textAlign: "center", fontFamily: "monospace",
                        fontSize: 12, fontWeight: 700, cursor: c.n ? "pointer" : "default",
                        background: isDiag ? "#eef1f4" : shade(c.n),
                        color: c.n && c.n / maxCount > 0.5 ? "#fff" : t.text,
                        border: `1px solid ${hovered ? t.accent : "#fff"}` }}>
                      {isDiag ? "\u2014" : (c.n || "")}
                    </td>
                  );
                })}
...
// ---- Focus view: pick one feed, show only its links grouped by key ----
function FocusView({ t, graph, domains, nodeById, sel, setSel }) {
  const focusId = sel && sel.type === "feed" ? sel.id : null;
  const focus = focusId ? nodeById(focusId) : null;

  if (!focus) {
    // picker: list feeds grouped by domain
    const byDom = {}; graph.nodes.forEach((n) => { (byDom[n.domain] = byDom[n.domain] || []).push(n); });
    return (
      <div style={{ padding: 16 }}>
        <div style={{ fontSize: 12, color: t.sub, marginBottom: 12 }}>
          Pick a {"feed"} to see only its direct connections, grouped by the linking key. No clutter.
        </div>
        {domains.map((d) => (
          <div key={d} style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: domColor(domains, d), marginBottom: 5 }}>{d}</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {(byDom[d] || []).map((n) => (
                <button key={n.id} onClick={() => setSel({ type: "feed", id: n.id })}
                  style={{ fontSize: 12, padding: "6px 11px", borderRadius: t.radius.md, cursor: "pointer",
                    border: `1px solid ${t.border}`, background: t.panel, color: t.text }}>{n.name}</button>
              ))}
            </div>
          </div>
        ))}
  ...
  // group this feed's edges by linking key
  const conns = graph.edges.map((e, i) => ({ e, i }))
    .filter(({ e }) => e.a === focus.id || e.b === focus.id);
  const byKey = {};
  conns.forEach(({ e, i }) => { (byKey[e.key] = byKey[e.key] || []).push({ e, i }); });

  return (
    <div style={{ padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={() => setSel(null)}
          style={{ fontSize: 11, border: `1px solid ${t.border}`, background: t.panel, borderRadius: t.radius.md,
            padding: "4px 10px", cursor: "pointer", color: t.sub }}>{"\u2190"} all feeds</button>
        <span style={{ fontSize: 16, fontWeight: 700 }}>{focus.name}</span>
        <span style={{ width: 9, height: 9, borderRadius: 5, background: domColor(domains, focus.domain) }} />
        <span style={{ fontSize: 12, color: t.sub }}>{focus.domain}</span>
        <span style={{ marginLeft: "auto", fontSize: 12, color: t.textMuted }}>{conns.length} connection{conns.length === 1 ? "" : "s"}</span>
      </div>

      {Object.keys(byKey).length === 0 && (
        <div style={{ color: t.textMuted, fontSize: 13 }}>No shared-key connections for this feed.</div>
      )}
      {Object.entries(byKey).map(([k, list]) => {
        const mismatch = list.some(({ e }) => e.match === "mismatch");
        return (
          <div key={k} style={{ marginBottom: 18 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <span style={{ fontFamily: "monospace", fontSize: 13, fontWeight: 700, color: mismatch ? "#a16207" : t.accent }}>
                via {k}</span>
              <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", padding: "1px 6px", borderRadius: 3,
                background: mismatch ? "#ca8a04" : t.success }}>{mismatch ? "\u26A0 CHECK" : "MATCH"}</span>
              <span style={{ fontSize: 11, color: t.textMuted }}>{list.length} feed{list.length === 1 ? "" : "s"}</span>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, paddingLeft: 12 }}>
              {list.map(({ e, i }) => {
                const other = nodeById(e.a === focus.id ? e.b : e.a); if (!other) return null;
                return (
                  <button key={i} onClick={() => setSel({ type: "feed", id: other.id })}
                    style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12.5, padding: "8px 12px",
                      borderRadius: t.radius.md, cursor: "pointer", background: t.panel,
                      border: `1px solid ${t.border}`, borderLeft: `3px solid ${domColor(domains, other.domain)}` }}>
                    <span>{other.name}</span>
                    <span style={{ fontSize: 10, color: t.textMuted }}>{other.domain}</span>
                  </button>
                );
              })}
            </div>
