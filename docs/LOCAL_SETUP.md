...
Connect as the user in `CP_CATALOG_DB_DSN`, then run **01 through 16 in order**:
...
...
sqlplus catalog_user/pw@... @sql\16_pipeline_builder.sql
```
All DDL is **idempotent** (guarded against ORA-00955 "already exists"), so re-running
is safe. Confirm key tables:
```sql
SELECT table_name FROM user_tables
WHERE table_name IN ('DATASETS','COLUMNS','TRANSFORM_LINEAGE','COLUMN_LINEAGE',
 'RUNS','API_ENDPOINTS','API_FIELDS','API_BUSINESS_FLOWS','INTERFACE360_SYSTEMS',
 'DP_REGISTRY','DP_OCCURRENCES','FEED_CATALOG','LOADER_CATALOG','DATA_PIPELINES',
 'BUSINESS_GLOSSARY','TERM_COLUMN_MAP')
ORDER BY table_name;
```
...
The 13 steps run in this order (each is idempotent):
```
projects → oracle → feed_dictionary → feed_catalog → loader_catalog →
dbt → glossary → airflow → interface360 → api360 →
pii_classification → pii_match → datapoint_index (LAST)
```
`datapoint_index` runs last on purpose — it scans everything the other steps loaded.
