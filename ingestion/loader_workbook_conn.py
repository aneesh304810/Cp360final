...
class LoaderWorkbookConnector:
    ...
    def parse(self) -> dict:
        ...
        # --- Errors_Exceptions (tolerant of sheet + column name variants) ---
        seqe = {}
        exc_sheet = sheet("Errors_Exceptions", "Errors Exceptions", "Exceptions",
                          "Errors", "Error_Handling", "Error Handling",
                          "Exception_Handling", "Exception Handling")
        exc_rows = list(rows_of(exc_sheet))
        for r in exc_rows:
            ln = pick(r, "loader_name", "loader", "loader name", "name")
            et = pick(r, "exception_type", "exception", "error_type", "error type",
                      "error", "error_code", "error code", "type")
            if not ln or not et:
                continue
            lid = _lid(ln)
            seqe[lid] = seqe.get(lid, 0) + 1
            excs.append({"loader_id": lid, "exception_type": et[:200],
                         "description": pick(r, "description", "error_message",
                                             "error message", "message", "details")[:1000] or None,
                         "resolution_path": pick(r, "resolution_path", "resolution",
                                                 "resolution steps", "action", "fix")[:1000] or None,
                         "seq": seqe[lid]})
        log.info("loader workbook: Errors_Exceptions sheet had %d rows, %d parsed to exceptions",
                 len(exc_rows), len(excs))
